...
## Acceptance Criteria
- [ ] Backend `POST /api/queue/reorder` endpoint atomically swaps queued tracks up or down under thread lock.
- [ ] Karaoke sidebar positions Playback Queue at the top and Song Library below.
- [ ] Queued song cards include `▲` and `▼` buttons that dynamically update order in real time.
- [ ] Moving a song to the top of the queue immediately updates the "Up Next" stage header.
- [ ] Top application header smoothly hides off-screen and reveals when the cursor hovers near the top viewport edge.
