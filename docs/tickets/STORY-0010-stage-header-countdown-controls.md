...
## Acceptance Criteria
- [ ] Karaoke stage header displays Title, Artist, total time, and remaining time countdown badge.
- [ ] When upcoming queued songs exist, header smoothly cross-fades between "Now Singing" and "Up Next" on a configurable interval (default 6s).
- [ ] Visual countdown cue (`● ○ ○` -> `● ● ○` -> `● ● ●`) pulses 3 seconds prior to the first lyric line and after instrumental interludes > 5s.
- [ ] Users can scale lyric font size dynamically via `A-` / `A+` buttons on the stage toolbar.
- [ ] Stage Settings modal allows adjusting transition interval, highlight color, and base font size with persistent `localStorage` storage.
