...
## Acceptance Criteria
- [ ] Top stage header displays both "Now Singing: [Title] - [Artist]" on the left and "Up Next: [Next Title] - [Next Artist]" on the right simultaneously.
- [ ] When no songs are queued, the right header displays a clear placeholder ("Up Next: — (Queue Empty)").
- [ ] Text overflowing header boundaries animates with a smooth CSS marquee effect.
- [ ] `A-` / `A+` buttons removed from stage header (font scaling consolidated in Stage Settings).
- [ ] Clicking the lyrics stage background toggles Play/Pause.
- [ ] Clicking a lyric line seeks directly to that line timestamp without triggering the stage play/pause toggle.
